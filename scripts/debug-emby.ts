...
import { EmbyClient, type EmbyConfig } from "../src/lib/sdk/emby";

const run = custom2;

async function _custom3(client: EmbyClient) {
...
async function _custom1(client: EmbyClient) {
	log("=== 开始执行 ===\n");
	try {
		const serverItems = await client.getItems({
			Recursive: true,
		...
		// 按名称分组，检测分段电影
		const [filteredItems, multiPartMovies] =
			EmbyClient.filterMultiPartItems(movieItems);
