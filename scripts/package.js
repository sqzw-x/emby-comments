...
async function packageApp() {
	...
	if (fs.existsSync(standaloneDir)) {
		fs.cpSync(standaloneDir, buildDir, {
			...
			filter: (raw, _target) => !raw.endsWith(".env"),
		});
		console.log("✅ Standalone files copied");
	} else {
		console.warn(
			'⚠️ Standalone build not found. Make sure output: "standalone" is set in next.config.js',
		);
	}
	...
	if (fs.existsSync(prismaDir)) {
		fs.cpSync(prismaDir, buildPrismaDir, {
			...
			filter: (raw, _target) =>
				!raw.endsWith(".db") &&
				!raw.endsWith(".db-journal") &&
				!raw.endsWith(".db-wal"), // 过滤掉数据库文件和 .env 文件
		});
	...
	try {
		...
		const nextEnvDir = fs
			.readdirSync(pnpmDir)
			.find((file) => file.startsWith("@next+env@"));
		if (!nextEnvDir)
			throw new Error(`@next+env@* does not exist in ${pnpmDir}`);
		const pkgRealPath = path.join(
			pnpmDir,
			nextEnvDir,
			"node_modules/@next/env",
		);
		if (!fs.existsSync(pkgRealPath))
			throw new Error(`${pkgRealPath} does not exist`);
