...
import type { LocalItem, Rating, Tag } from "@prisma/client";
...
const itemColumns: GridColDef<LocalItemWithRelations>[] = [
	...
	},
	{
		...
		renderCell: (params) => (
			<Link
				href={Routes.itemDetail({ id: params.row.id.toString() })}
				target="_blank"
			>
...
export function ItemsAdmin({ localItems, allTags }: ItemsAdminProps) {
	...
	return (
		<>
			...
			>
				<DialogTitle>
					{tagDialogState.adding ? "添加标签到选定项目" : "从选定项目移除标签"}
				</DialogTitle>
				<DialogContent>
					<Box sx={{ pt: 1 }}>
									...
									: // 移除标签时只显示已选项目上的标签
										(() => {
											...
											return tagIds
												...
												.map(
													(id) =>
														// biome-ignore lint/style/noNonNullAssertion: tagIds 元素必然存在于 tags
