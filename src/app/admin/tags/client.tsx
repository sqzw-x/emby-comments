...
const tagColumns: GridColDef<TagWithCount>[] = [
	{ field: "name", headerName: "名称", width: 200 },
	{
		...
		headerName: "项目数",
		width: 150,
		valueGetter: (_, row) => row._count.items,
	},
	...
	},
	{
		...
		renderCell: (params) => (
			<Link
				href={Routes.tagDetail({ id: params.row.id.toString() })}
				target="_blank"
			>
...
export function TagsAdmin({ initData, groups }: TagsAdminProps) {
	...
	const handleConfirmSetGroup = useCallback(async () => {
		...
		if (res.success) {
			showSuccess(
				`成功设置 ${selectedRows.size} 个标签的分组为 "${groupValue}"`,
			);
		} else {
	...
	return (
		<>
			...
			{/* 设置分组对话框 */}
			<Dialog
				open={setGroupDialogOpen}
				onClose={handleCancelSetGroup}
				maxWidth="sm"
				fullWidth
			>
