...
export default function CommentsClient({
	...
	const filteredComments = useMemo(() => {
		return comments.filter((comment) => {
			...
			const titleMatch = comment.localItem?.title
				.toLowerCase()
				.includes(searchTerm.toLowerCase());
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: 当搜索条件变化时重置页码
	React.useEffect(() => {
		setCurrentPage(1);
	}, [searchTerm]);
											...
											{comment.localItem && (
												<MuiButton
													...
													onClick={() =>
														router.push(
															Routes.itemDetail({
																id: comment.localItem.id.toString(),
															}),
														)
													}
												>
