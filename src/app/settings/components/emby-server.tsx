	...
	return (
		...
		>
			...
			<DialogContent>
				<Stack spacing={3} sx={{ mt: 1 }}>
					...
					<TextField
						...
						helperText={
							formErrors.url || "完整URL地址，包括http://或https://和端口号"
						}
						fullWidth
						disabled={isLoading}
					/>
					...
					<TextField
						...
						helperText={
							formErrors.apiKey || "在Emby服务器中生成的API密钥，用于授权访问"
						}
						fullWidth
						disabled={isLoading}
					/>
			...
			<DialogActions>
				...
				<Button
					onClick={handleTest}
					disabled={isLoading || !formData.url || !formData.apiKey}
					variant="outlined"
				>
					{isLoading ? "测试中..." : "测试连接"}
				</Button>
...
export default function EmbyServerSetting({
	servers,
	onServersChange,
}: EmbyServerSettingProps) {
