...
import type { ExternalLinkProvider } from "@prisma/client";
...
export default function ExternalLinkSetting({
	providers,
	onProvidersChange,
}: ExternalLinkSettingProps) {
	const [isDialogOpen, setIsDialogOpen] = useState(false);
	const [editingProvider, setEditingProvider] =
		useState<ExternalLinkProvider | null>(null);
	...
	const validateForm = (): boolean => {
		...
		if (!formData.key) {
			...
			const existingProvider = providers.find(
				(p) =>
					p.key === formData.key.toLowerCase() && p.id !== editingProvider?.id,
			);
			if (existingProvider) {
				errors.key = "此键已存在";
			}
	...
	const handleSubmit = async () => {
		...
		if (editingProvider) {
			// 更新
			const result = await updateExternalLinkProvider(
				editingProvider.id,
				formData,
			);
			if (result.success) {
	...
	const handleToggleEnabled = async (provider: ExternalLinkProvider) => {
		...
		if (result.success) {
			showSuccess(
				`${provider.name} ${provider.isEnabled ? "已禁用" : "已启用"}`,
			);
			onProvidersChange();
		} else {
	...
	const testTemplate = (template: string) => {
		...
		return template.replace("{value}", "tt0111161");
	};
	...
	return (
		<Box>
			...
			<Alert severity="info" sx={{ mb: 3 }}>
				外部链接提供商用于生成项目详情页中的外部链接。URL模板中使用 {"{value}"}{" "}
				作为外部ID的占位符。
			</Alert>
									...
									<TableCell>
										<Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
											...
											{testTemplate(provider.template) && (
												<IconButton
													size="small"
													// biome-ignore lint/style/noNonNullAssertion: 已判断
			...
			>
				<DialogTitle>
					{editingProvider ? "编辑外部链接提供商" : "添加外部链接提供商"}
				</DialogTitle>
				<DialogContent>
					<Stack spacing={3} sx={{ mt: 1 }}>
						...
						<TextField
							...
							helperText={
								formErrors.key ||
								"唯一标识符，只能包含字母、数字、下划线和连字符，不区分大小写"
							}
							fullWidth
							disabled={isLoading}
						/>
						...
						<TextField
							...
							helperText={
								formErrors.template ||
								"URL模板，使用 {value} 作为外部ID占位符，如 https://www.imdb.com/title/{value}"
							}
							fullWidth
							multiline
							rows={2}
