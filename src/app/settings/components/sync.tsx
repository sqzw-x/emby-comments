...
export default function ServerSync({ server, onClose }: ServerSyncProps) {
	...
	// 复选框选择状态
	const [selectedItems, setSelectedItems] = useState<Set<ExtendedResult>>(
		new Set(),
	);
	...
	// 自定义映射对话框状态
	const [customMapItem, setCustomMapItem] = useState<ExtendedResult | null>(
		null,
	);
	...
	const filterAndSortItems = useCallback(
		(items: ExtendedResult[]) => {
			...
			const filtered = items.filter((item) => {
				...
				const matchesType =
					typeFilter === "all" || item.item.type === typeFilter;
			...
			return [...filtered].sort((a, b) => {
				...
				switch (sortBy) {
					...
					case "score":
						aVal =
							a.matches.length > 0
								? Math.max(...a.matches.map((m) => m.score))
								: 0;
						bVal =
							b.matches.length > 0
								? Math.max(...b.matches.map((m) => m.score))
								: 0;
						break;
					default:
						return 0;
	...
	const syncItems = useMemo(() => {
		const exact = filterAndSortItems(
			results.filter((item) => item.status === "exact"),
		);
		const multiple = filterAndSortItems(
			results.filter((item) => item.status === "multiple"),
		);
		const none = filterAndSortItems(
			results.filter((item) => item.status === "none"),
		);
		const matched = filterAndSortItems(
			results.filter((item) => item.status === "matched"),
		);
	...
	const doStartSync = useCallback(async () => {
		...
		if (syncRes.success) {
			...
			if (
				syncRes.value.reduce(
					(total, item) => total + (item.status !== "matched" ? 1 : 0),
					0,
				) === 0
			) {
				...
				toast.showSuccess(
					`同步完成，发现 ${syncRes.value.length} 个需要处理的项目`,
				);
			}
	...
	const addPendingAction = useCallback(
		(action: ItemMapOperation, item: ExtendedResult) => {
	...
	// 移除待处理操作
	const removePendingAction = useCallback((item: ExtendedResult) => {
	...
	// 选择最佳匹配
	const handleSelectBestMatch = useCallback(
		(item: ExtendedResult) => {
			if (item.matches.length === 0) return;
			const bestMatch = item.matches.reduce((best, current) =>
				current.score > best.score ? current : best,
			);
			addPendingAction(
				{ type: "map", embyItemId: item.item.id, localItemId: bestMatch.id },
				item,
			);
		},
		[addPendingAction],
	...
	const handleSelectMatch = useCallback(
		(item: ExtendedResult, localItem: LocalItem) => {
			item.selected = localItem;
			addPendingAction(
				{ type: "map", embyItemId: item.item.id, localItemId: localItem.id },
				item,
			);
		},
		[addPendingAction],
	...
	const handleRefresh = useCallback(
		(item: ExtendedResult) =>
			addPendingAction({ type: "refresh", embyItemId: item.item.id }, item),
		[addPendingAction],
	);
	...
	const handleUnmap = useCallback(
		(item: ExtendedResult) =>
			addPendingAction({ type: "unmap", embyItemId: item.item.id }, item),
		[addPendingAction],
	);
	// 批量选择最佳匹配
	...
	const handleBatchCancel = useCallback(() => {
		...
		setSelectedItems(new Set());
	}, [selectedItems, removePendingAction]);
	...
	const toggleSelectAll = useCallback(() => {
		const notAllSelected = currentTabItems.some(
			(item) => !selectedItems.has(item),
		);
		setSelectedItems(() =>
			notAllSelected ? new Set(currentTabItems) : new Set(),
		);
	...
	const renderPendingItem = useCallback(
		...
		[selectedItems, toggleSelection, removePendingAction],
	);
				...
				case "exact":
					return (
						<ResultList
							...
							batchActions={[
								{
									onClick: handleBatchSelectBestMatch,
									label: "选择最佳匹配",
									icon: Lucide.Check,
								},
							]}
		...
		() =>
			new Map(
				StatusMap.keys().map((status) => [
					status,
					<ItemList key={status} status={status} />,
				]),
			),
		[ItemList],
	);
									...
									>
										...
										<FormControl size="small" fullWidth>
											<Select
												...
												onChange={(e: SelectChangeEvent) =>
													setHasMatchesFilter(e.target.value)
												}
												displayEmpty
												MenuProps={{ disableScrollLock: true }}
											>
