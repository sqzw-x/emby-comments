...
import type { EmbyServer, Prisma } from "@prisma/client";
import {
	ArrowLeft,
	Check,
	Edit,
...
export default function TagDetailClient({
	...
	const handleUpdateTag = async () => {
		...
		const result = await updateTag(
			tag.id,
			editingTag.name,
			editingTag.description ?? undefined,
			editingTag.group,
		);
		if (result.success) {
			setIsEditing(false);
							...
							{isEditing ? (
								...
								<Stack spacing={2}>
									...
									<TextField
										...
										onChange={(e) =>
											updateEditingField("group", e.target.value)
										}
										label="标签分组"
										variant="outlined"
										size="small"
