...
export default function TagsClient({ allTags }: TagsClientProps) {
	...
	const numberDeserializer = (value: string): number | "" =>
		value === "" ? "" : Number(value);
	const [minCount, setMinCount] = useLocalStorage<number | "">(
		"tags.minCount",
		0,
		{
	...
	const [maxCount, setMaxCount] = useLocalStorage<number | "">(
		"tags.maxCount",
		"",
		{
			deserializer: numberDeserializer,
		...
		<>
			...
			<Box sx={{ mb: 2 }}>
				<Typography variant="body2" color="text.secondary">
					...
					{stats.filteredCount !== allTags.length && (
						<span> · 已应用筛选条件</span>
					)}
				</Typography>
			</Box>
			...
			<ContentArea
				...
				emptyMessage={
					searchTerm || stats.activeFilters.length > 0
						? `没有找到符合条件的标签`
						: "还没有创建任何标签"
				}
				skeletonType="cards"
