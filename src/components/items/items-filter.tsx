...
import type { TagWithCount } from "@/lib/service/tag";
...
export type ItemsFilterProps = {
	...
	onFilterChange: (
		newFilters: { [P in keyof Query]?: Query[P] | null },
	) => void;
};
...
export function ItemsFilter({
	...
	const [localYearFrom, setLocalYearFrom] = useState(
		searchOption.yearFrom ?? null,
	);
	const [localYearTo, setLocalYearTo] = useState(searchOption.yearTo ?? null);
	// 标签分组展开状态管理
	const [expandedGroups, setExpandedGroups] = useLocalStorage<Set<string>>(
		"items.filter.expandedGroups",
		new Set(),
		{
	...
	const groupedTags = useMemo(() => {
		const groups: {
			[key: string]: Array<{ id: number; name: string; group?: string | null }>;
		} = {};
	...
	const hasActiveFilters =
		searchOption.yearFrom ||
		searchOption.yearTo ||
		(searchOption.tagIds && searchOption.tagIds.length > 0);
	...
	const handleTagToggle = (tagId: number) => {
		...
		const tagIds = raw.includes(tagId)
			? raw.filter((t) => t !== tagId)
			: [...raw, tagId];
												...
												<Box key={groupName}>
													...
													<Collapse in={isExpanded}>
														...
														>
															{groupTags.map((tag) => {
																const isSelected =
																	searchOption.tagIds?.includes(tag.id) ||
																	false;
																return (
																	<Chip
																		key={tag.id}
							...
							<Stack direction="row" spacing={1} flexWrap="wrap">
								...
								{searchOption.tagIds?.map((tagId) => (
									<Chip
										...
										onDelete={() => {
											const newTags =
												searchOption.tagIds?.filter((id) => id !== tagId) || [];
