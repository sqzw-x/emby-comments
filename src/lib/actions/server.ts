...
import {
	...
	getActiveServerCached,
	updateActiveServerCache,
} from "../cache/server-cache";
import { EmbyClient } from "../sdk/emby";
import { itemService } from "../service/item";
import { type ServerCreateData, serverService } from "../service/server";
import { action } from "./utils";

/**
 * 获取所有服务器
 */
export const getAllServers = action(
	serverService.getAllServers.bind(serverService),
);

/**
 * 获取服务器详情
 */
export const getServerById = action(
	serverService.getServerById.bind(serverService),
);
...
export const updateServer = action(
	async (id: number, data: Partial<ServerCreateData>) => {
 ...
 * 测试服务器连接
 */
export const testServerConnection = action(
 ...
 * 批量操作
 */
export const batchProcessMappings = action(
	itemService.batchProcessMappings.bind(itemService),
);

/**
 * 获取未映射的本地项目
 */
export const getUnmappedLocalItems = action(
	itemService.getUnmappedLocalItems.bind(itemService),
);
