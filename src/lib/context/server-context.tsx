...
export function ServerProvider({
  ...
  const [activeServer, setActiveServerState] = useLocalStorage<EmbyServer | null>("activeServer", initialActiveServer);

  // 如果服务端提供的初始服务器与本地存储不同，则更新本地存储
  useEffect(() => {
    if (initialActiveServer && (!activeServer || initialActiveServer.id !== activeServer.id)) {
