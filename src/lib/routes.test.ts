...
describe("buildSearchParams", () => {
  ...
  it("处理字符串和数组的混合参数", () => {
    const result = buildSearchParams({ search: "test", tags: ["action", "drama"], category: "movies" });
    expect(result).toBe("?search=test&tags=action&tags=drama&category=movies");
  });

  it("过滤 undefined 值", () => {
    const result = buildSearchParams({ search: "test", category: undefined, tags: ["action"] });
    expect(result).toBe("?search=test&tags=action");
  });

  it("过滤 null 值", () => {
    const result = buildSearchParams({ search: "test", category: null as any, tags: ["action"] });
    expect(result).toBe("?search=test&tags=action");
  });

  it("过滤空串", () => {
    const result = buildSearchParams({ search: "test", category: "", tags: ["action"] });
    expect(result).toBe("?search=test&tags=action");
  });

  it("从数组中过滤 null/undefined/空串", () => {
    const result = buildSearchParams({ search: "test", tags: ["action", undefined, "", null, "drama"] as any });
    expect(result).toBe("?search=test&tags=action&tags=drama");
  });

  it("处理值中的特殊字符", () => {
    const result = buildSearchParams({ search: "test & special chars", category: "sci-fi" });
  ...
  it("处理所有值都为 null/undefined/空串 的数组", () => {
    const result = buildSearchParams({ search: "test", tags: [undefined, null, ""] as any });
    expect(result).toBe("?search=test");
  });
