...
import type { BaseItem, ItemListParams } from "emby-sdk-stainless/resources";
...
export class EmbyClient extends Emby {
	...
	async getAdminUserId() {
		...
		this.userId = users.Items?.filter((u) => u.Policy?.IsAdministrator).at(
			0,
		)?.Id;
		return this.userId;
	}
	...
	static filterMultiPartItems(allItems: BaseItem[]) {
		...
		const multiPartMovies = groupByName
			...
			.map((movies) =>
				Map.groupBy(movies, (item) => (item.Path ? getDir(item.Path) : "_")),
			)
			.reduce((acc, group) => {
		...
		// 分段仅保留一部分
		const needRemove = multiPartMovies.flatMap((items) => items.slice(1));
