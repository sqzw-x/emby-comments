...
import type { BaseItem } from "emby-sdk-stainless/resources";
...
export class ItemService {
	...
	async createOrUpdateEmbyItem(
		...
		const actors = arrayToDbString(
			embyItem.People?.filter((p) => p.Type === "Actor").map((a) => a.Name),
		);
		const directors = arrayToDbString(
			embyItem.People?.filter((p) => p.Type === "Director").map((d) => d.Name),
		);

		// 转换日期
		const dateAdded = embyItem.DateCreated
			? new Date(embyItem.DateCreated)
			: null;
		const premiereDate = embyItem.PremiereDate
			? new Date(embyItem.PremiereDate)
			: null;
		// biome-ignore-start lint/style/noNonNullAssertion: API 返回值必然存在相关字段
		const data = {
			title: embyItem.Name!,
			...
			directors,
			productionYear:
				embyItem.ProductionYear || premiereDate?.getFullYear() || null,
			dateAdded,
			communityRating: embyItem.CommunityRating || null,
			embyCreateAt: embyItem.DateCreated
				? new Date(embyItem.DateCreated)
				: null, // Emby 上的创建时间
		};
		// 更新或创建Emby项目
		const updatedEmbyItem = await dbClient.embyItem.upsert({
			where: {
				embyId_embyServerId: { embyId: embyItem.Id!, embyServerId: serverId },
			},
			update: data,
	...
	async syncEmby(server: EmbyServer): Promise<ItemSyncResult[]> {
		...
		const serverItems = await embyClient.getItems({
			...
			Fields:
				"Overview,Genres,Studios,ProviderIds,DateCreated,People,Path,PremiereDate,ProductionYear",
			SortBy: "Name",
			SortOrder: "Ascending",
		});
		...
		const allEmbyItems = await Promise.all(
			filteredItems.map((item) =>
				this.createOrUpdateEmbyItem(server.id, item, true),
			),
		);
		if (
			allEmbyItems.length !==
			(await dbClient.embyItem.count({ where: { embyServerId: server.id } }))
		) {
			// 如果数量不一致，可能是有删除的项目
			// 删除当前服务器上不存在的 ID
			const { count } = await dbClient.embyItem.deleteMany({
				where: {
					...
					embyId: { notIn: allEmbyItems.map((item) => item.embyId) },
				},
			});
	...
	private matchItems(
		...
		out: for (const embyItem of allEmbyItems) {
			...
			res.push({
				item: embyItem,
				matches,
				status: matches.length > 0 ? "multiple" : "none",
			});
		}
		return res;
	}
	...
	async createLocalItemFromEmby(id: number) {
		...
		return await dbClient.$transaction(async (tx) => {
			...
			const localItem = await tx.localItem.create({
				data: this.getLocalData(item),
			});
			// 添加标签
			await this.autoTag(tx, localItem, item);
			// 创建关联
			await tx.embyItem.update({
				where: { id: item.id },
				data: { localItem: { connect: { id: localItem.id } } },
			});
			return localItem;
		});
	}
	...
	async updateLocalItem(embyItemId: number) {
		...
		return await dbClient.localItem.update({
			where: { id },
			data: this.getLocalData(embyItem),
		});
	}
	/**
	 * 手动映射本地项目和Emby项目
	...
	async unmapItem(embyItemId: number) {
		await dbClient.embyItem.update({
			where: { id: embyItemId },
			data: { localItem: { disconnect: true } },
		});
	}
	...
	async filterLocalItems(query: ItemSearchOptions, serverId: number) {
		...
		if (query.yearFrom) {
			whereConditions.premiereDate.gte = query.yearFrom
				? new Date(query.yearFrom, 0, 1)
				: undefined;
		}
		if (query.yearTo) {
			whereConditions.premiereDate.lte = query.yearTo
				? new Date(query.yearTo, 11, 31)
				: undefined;
		}

		// 标签列表过滤
		if (query.tagIds && query.tagIds.length > 0) {
			whereConditions.AND = query.tagIds.map((tagId) => ({
				tags: { some: { id: tagId } },
			}));
		}
		...
		const sortBy =
			query.sortBy === "embyCreatedAt" ? "title" : (query.sortBy ?? "title");
		...
		const args = {
			...
			include: {
				tags: true,
				rating: true,
				embyItems: { where: { embyServerId: serverId } },
			},
		};
	...
	async batchRemoveTagsFromItems(itemIds: Iterable<number>, tagIds: number[]) {
		...
		for (const itemId of itemIds) {
			operations.push(
				dbClient.localItem.update({
					where: { id: itemId },
					data: { tags: { disconnect: tagDisconnections } },
				}),
			);
		}
		await dbClient.$transaction(operations);
	 ...
	 * - 添加演员/导演/厂商标签, 不存在则新建.
	 */
	private async autoTag(
		tx: Prisma.TransactionClient,
		localItem: LocalItem & { tags?: Tag[] },
		embyItem: EmbyItem,
	) {
		...
		await Promise.all([
			...
			Promise.all(
				actors.map(async (actor) =>
					this.addTagToItem(tx, localItem.id, `演员: ${actor}`),
				),
			),
			Promise.all(
				directors.map(async (director) =>
					this.addTagToItem(tx, localItem.id, `导演: ${director}`),
				),
			),
			Promise.all(
				studios.map(
					async (studio) =>
						await this.addTagToItem(tx, localItem.id, `片商: ${studio}`),
				),
			),
		]);
	}

	private async addTagToItem(
		tx: Prisma.TransactionClient,
		localItemId: number,
		tagName: string,
	) {
