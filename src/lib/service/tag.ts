...
export class TagService {
	...
	async updateTag(
		...
		// 检查标签名是否已存在
		const existingTag = await dbClient.tag.findFirst({
			where: { name, id: { not: id } },
		});
		if (existingTag) {
			throw new Error(`标签名 "${name}" 已存在，请使用其他名称。`);
		}
		return dbClient.tag.update({
			where: { id },
	...
	async removeTagFromItem(localItemId: number, tagId: number) {
		return dbClient.localItem.update({
			where: { id: localItemId },
	...
	async getTagsByServerId(
		...
		return await dbClient.tag.findMany({
			where: {
				items: { some: { embyItems: { some: { embyServerId: serverId } } } },
			},
			include: { _count: { select: { items: true } } },
			orderBy: order,
		});
	...
	async getTagWithItems(tagId: number, serverId?: number) {
		const tag = await dbClient.tag.findUnique({
			...
			include: {
				items: {
					where: serverId
						? { embyItems: { some: { embyServerId: serverId } } }
						: {},
					include: {
						tags: true,
						rating: true,
						embyItems: serverId ? { where: { embyServerId: serverId } } : true,
					},
				},
			...
			for (const item of sourceTagItems) {
				...
				if (!hasTargetTag) {
					await tx.localItem.update({
						where: { id: item.id },
						data: { tags: { connect: { id: targetTagId } } },
					});
				}
			}
			// 3. 获取标签描述信息
			const sourceTags = await tx.tag.findMany({
				where: { id: { in: [...ids, targetTagId] } },
			});
			let desc = "";
