...
describe("数据库转换辅助函数", () => {
	describe("arrayToDbString", () => {
		it("应该正确转换数组为字符串", () => {
			...
			expect(arrayToDbString([{ id: 1 }, { id: 2 }])).toBe(
				'[{"id":1},{"id":2}]',
			);
		});
	...
	describe("dbStringToArray", () => {
		it("应该正确转换字符串为数组", () => {
			...
			expect(dbStringToArray<{ id: number }>('[{"id":1},{"id":2}]')).toEqual([
				{ id: 1 },
				{ id: 2 },
			]);
		});
		...
		it("应该处理无效的 JSON 字符串", () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith(
				"Failed to parse array from database string:",
				expect.any(Error),
			);
	...
	describe("dbStringToMap", () => {
		...
		it("应该处理无效的 JSON 字符串", () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith(
				"Failed to parse Map from database string:",
				expect.any(Error),
			);
			consoleSpy.mockRestore();
		});
	...
	describe("dbStringToObject", () => {
		...
		it("应该处理无效的 JSON 字符串", () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith(
				"Failed to parse object from database string:",
				expect.any(Error),
			);
			consoleSpy.mockRestore();
		});
	...
	describe("dbStringToJson", () => {
		...
		it("应该处理无效的 JSON 字符串", () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith(
				"Failed to parse JSON from database string:",
				expect.any(Error),
			);
			consoleSpy.mockRestore();
		});
	...
	describe("dbStringToJsonWithValidation", () => {
		...
		it("应该正确转换并验证数据", () => {
			expect(dbStringToJsonWithValidation('"test"', isString, "fallback")).toBe(
				"test",
			);
			expect(dbStringToJsonWithValidation("123", isNumber, 0)).toBe(123);
			expect(
				dbStringToJsonWithValidation('{"id":1,"name":"test"}', isUser, {
					id: 0,
					name: "",
				}),
			).toEqual({
		...
		it("验证失败应返回", () => {
			const consoleSpy = jest
				.spyOn(console, "warn")
				.mockImplementation(() => {});
			expect(dbStringToJsonWithValidation("123", isString, "fallback")).toBe(
				"fallback",
			);
			consoleSpy.mockRestore();
		});
		...
		it("应该处理空字符串和 null", () => {
			expect(dbStringToJsonWithValidation(null, isString, "fallback")).toBe(
				"fallback",
			);
			expect(dbStringToJsonWithValidation("", isString, "fallback")).toBe(
				"fallback",
			);
		});
		...
		it("应该处理无效的 JSON 字符串", () => {
			const consoleSpy = jest
				...
				.mockImplementation(() => {});
			expect(
				dbStringToJsonWithValidation("invalid json", isString, "fallback"),
			).toBe("fallback");
			expect(consoleSpy).toHaveBeenCalledWith(
				"Failed to parse JSON from database string:",
				expect.any(Error),
	...
	describe("dbStringToSet", () => {
		...
		it("应该处理无效的 JSON 字符串", () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith(
				"Failed to parse Set from database string:",
				expect.any(Error),
			);
			consoleSpy.mockRestore();
		});
