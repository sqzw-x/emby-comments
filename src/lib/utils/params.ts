...
export type SafeSearchParam<T extends Record<string, string | number | number[] | string[] | undefined>> = {
  [K in keyof T]: NumberToString<string[] extends T[K] ? string | T[K] : T[K]>;
};

type NumberToString<T> = T extends number ? string : T extends number[] ? string[] : T;
...
// 字段转换配置类型
export interface ParamConfig<T extends Record<string, string | number | number[] | string[] | undefined>> {
  /** 需要转换为 number 的字段 */
...
// 转换函数
export function parseSearchParams<T extends Record<string, string | number | number[] | string[] | undefined>>(
  params: SafeSearchParam<T>,
...
export function toSearchParams<T extends Record<string, string | number | number[] | string[] | undefined>>(
  params: T
): SafeSearchParam<T> {
